...
module Airity
  class HtmlGenerator
    ...
    def form_for(model_name, action = nil, id = nil, token = nil)
      ...
      if token
        hidden = @xdoc.create_element('input')
        hidden.append_attribute(@xdoc.create_attribute('name', 'authenticity_token'))
        hidden.append_attribute(@xdoc.create_attribute('type', 'hidden'))
        hidden.append_attribute(@xdoc.create_attribute('value', token))
        element.append_child(hidden)
      end
